...
twoway 	bar beta i, color(edkblue) barw(.5)  || ///
		...
		xlab(1 "Deaths" 2 "Cases" 3 "Hospitalizations" 4 `" "Cases" "delaying" "treatment" "' ///
		5 `" "Ave. days" "delayed" "',labsize(medlarge))	///
		xtitle("") legend(off) ylab(,labsize(medlarge)) ytitle("%{&Delta} Outcome from full insurance" "coverage",size(large) ) ysc(titlegap(-5))
graph export "Output/baseline.pdf", replace		
