...
forvalues c = 1/8{
	...
	* Cutoff date
	local cutoff = `ld'-`change'*(`c'-1)*2
	local month = month(`cutoff')
	local day = day(`cutoff')
	local year = year(`cutoff')
	...
	* Controls
	...
	if `c'==2{
		...
		estadd local hea= "\checkmark"
		estadd local com = "\checkmark"
		estadd local cut = "`month'/`day'/`year'"
	}
	if `c'==3{
		local controls="_d* _h* _c*"
		local absorb="stfips time agecat sexcat racecat"
	...
	frame put if time<=`cutoff', into(`tframe')
	frame `tframe'{
...
* Save Table
esttab col* using Output/report_delay.tex,				 							///
	stats(	blank b_death se_death N_death t_death								///
			...
			cut pop cases groups days insurance									///
		...
		"\addlinespace[0.3cm] \underline{\textit{Ave. days delayed from symptom onset}}" ///
			...
			"\addlinespace[0.2cm]\hspace{.25cm}Sample size"						///
			"\addlinespace[0.2cm]\hspace{.25cm}Total"							/// 	
		"\addlinespace[0.3cm] \midrule Cutoff date"					///
		"\addlinespace[0.1cm]Ave. group population"								///
